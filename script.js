// SkillWay - ПОЛНАЯ РАБОЧАЯ ВЕРСИЯ с Supabase
...
async function checkUserSession() {
    ...
    if (user) {
        const { data: profile } = await supabaseClient
            ...
            .eq('id', user.id)
            .single()
        currentUser = { ...user, ...profile, favorites: profile?.favorites || [], enrolled_courses: profile?.enrolled_courses || [] }
    }
    updateAuthUI()
}
...
async function renderAllCourses() {
    ...
    container.innerHTML = paginated.length ? paginated.map(course => createCourseCard(course)).join('') : '<div class="empty-state">Ничего не найдено</div>'
...
function createCourseCard(course) {
    ...
    const adminButtons = isAdmin ? `<div class="admin-edit-btn" onclick="event.stopPropagation(); openEditCourseModal(${JSON.stringify(course).replace(/"/g, '&quot;')})"><i class="fas fa-edit"></i> Ред.</div>
        <div class="admin-edit-btn" style="top:48px; background:#dc2626;" onclick="event.stopPropagation(); deleteCourse(${course.id})"><i class="fas fa-trash"></i> Уд.</div>` : ''
    
    return `<div class="course-card" data-id="${course.id}">
        <div class="course-image"><div class="image-fallback"><i class="fas fa-${icon}"></i></div>${discount > 0 ? `<span class="course-badge hot">-${discount}%</span>` : ''}</div>
        ${adminButtons}
        <div class="course-content">
            <span class="course-category">${course.category}</span>
            <h3 class="course-title">${escapeHtml(course.title)}</h3>
            <p class="course-description">${escapeHtml(course.description)}</p>
            <div class="course-meta"><span><i class="far fa-clock"></i> ${course.duration}</span><span><i class="fas fa-users"></i> ${course.students || 0}</span></div>
            <div class="course-footer"><div><span class="course-price">${course.price.toLocaleString()} ₽</span>${course.old_price ? `<span class="course-price-old">${course.old_price.toLocaleString()} ₽</span>` : ''}</div><div class="stars">${getStars(course.rating)}</div></div>
            <div style="margin-top:16px;display:flex;gap:10px;" onclick="event.stopPropagation()">
                ${isEnrolled ? `<button class="btn btn-primary btn-block" onclick="continueCourse(${course.id})">Продолжить</button>` : `<button class="btn btn-primary btn-block" onclick="openBookingModal(${course.id})">Записаться</button>`}
                <button class="favorite-btn ${isFavorite ? 'active' : ''}" onclick="toggleFavorite(${course.id})"><i class="fas fa-heart"></i></button>
            </div>
        </div>
    </div>`
}
...
async function renderMasterclasses() {
    ...
    if (!container) return
    const isAdmin = currentUser?.role === 'admin'
    container.innerHTML = allMasterclasses.map(mc => `<div class="course-card" data-id="${mc.id}">
        <div class="course-image"><div class="image-fallback"><i class="fas fa-chalkboard-user"></i></div><span class="course-badge hot">Мастер-класс</span></div>
        ${isAdmin ? `<div class="admin-edit-btn" onclick="event.stopPropagation(); openEditMasterclassModal(${JSON.stringify(mc).replace(/"/g, '&quot;')})"><i class="fas fa-edit"></i> Ред.</div>
            <div class="admin-edit-btn" style="top:48px; background:#dc2626;" onclick="event.stopPropagation(); deleteMasterclass(${mc.id})"><i class="fas fa-trash"></i> Уд.</div>` : ''}
        <div class="course-content"><span class="course-category">Интенсив</span><h3 class="course-title">${escapeHtml(mc.title)}</h3><p class="course-description">${escapeHtml(mc.description)}</p>
        <div class="course-meta"><span><i class="far fa-calendar"></i> ${new Date(mc.datetime).toLocaleDateString()}</span><span><i class="fas fa-hourglass-half"></i> ${mc.duration}</span></div>
        <div class="course-footer"><span class="course-price">${mc.price.toLocaleString()} ₽</span><button class="btn btn-primary" onclick="event.stopPropagation(); bookMasterclass(${mc.id})">Записаться</button></div></div></div>`).join('')
}
...
async function renderReviews() {
    ...
    if (currentReviewFilter !== 'all') reviews = reviews.filter(r => r.rating >= parseInt(currentReviewFilter))
    const start = (currentReviewsPage - 1) * REVIEWS_PER_PAGE
    const paginated = reviews.slice(start, start + REVIEWS_PER_PAGE)
    container.innerHTML = paginated.length ? paginated.map(r => `<div class="review-card"><div class="review-header"><div><span class="review-name">${escapeHtml(r.user_name)}</span><div class="review-date">${r.date}</div></div><div class="review-rating">${getStars(r.rating)}</div></div><p class="review-text">${escapeHtml(r.text)}</p></div>`).join('') : '<div class="empty-state">Пока нет отзывов</div>'
...
async function renderReviewsSummary() {
    ...
    container.innerHTML = `<div><span class="rating-number">${avg}</span><div class="stars">${getStars(avg)}</div></div><div>${allReviews.length} отзывов</div>`
}

function renderHeroStats() {
    const container = document.getElementById('heroStats')
    if (!container) return
    container.innerHTML = `<div class="hero-stat"><span class="hero-stat-number">${allCourses.length}</span><span class="hero-stat-label">курсов</span></div>
...
function renderAboutStats() {
    ...
    if (!container) return
    container.innerHTML = `<div class="stat-big"><span class="stat-number">5</span><span>+ лет</span><p>на рынке</p></div>
...
async function toggleFavorite(courseId) {
    ...
    if (isFavorite) favorites = favorites.filter(id => id !== courseId)
    else favorites.push(courseId)
    await supabaseClient.from('profiles').update({ favorites }).eq('id', currentUser.id)
    currentUser.favorites = favorites
    showNotification(isFavorite ? 'Удалено из избранного' : 'Добавлено в избранное')
    await renderAllCourses()
    if (document.getElementById('profile-page')?.classList.contains('active')) await renderProfileFavorites()
}
...
async function renderProfileCourses() {
    ...
    container.innerHTML = enrolled.map(c => `<div class="profile-list-item"><div><h4>${c.title}</h4><p>${c.instructor} • ${c.duration}</p><div class="progress-bar"><div class="progress-fill" style="width:0%"></div></div></div><div><span class="course-price">${c.price.toLocaleString()} ₽</span><button class="btn btn-primary btn-sm" onclick="continueCourse(${c.id})">Продолжить</button></div></div>`).join('')
}
...
async function renderProfileBookings() {
    ...
    container.innerHTML = bookings.map(b => `<div class="profile-list-item"><div><h4>${b.type === 'course' ? 'Курс' : 'МК'}: ${b.title}</h4><p>Статус: ${b.status === 'pending' ? 'Ожидает' : 'Подтвержден'}</p></div><div><span class="course-price">${b.price?.toLocaleString()} ₽</span></div></div>`).join('')
}
...
async function openBookingModal(courseId = null) {
    if (!currentUser) { showNotification('Войдите в аккаунт'); openModal('loginModal'); return }
    if (courseId) document.getElementById('bookingCourseSelect').value = courseId
...
async function submitBooking(e) {
    ...
    if (!course) { showNotification('Выберите курс'); return }
    const { error } = await supabaseClient.from('bookings').insert([{ user_id: currentUser.id, type: 'course', item_id: courseId, title: course.title, price: course.price, status: 'pending' }])
...
async function bookMasterclass(id) {
    ...
    if (!mc) return
    const { error } = await supabaseClient.from('bookings').insert([{ user_id: currentUser.id, type: 'masterclass', item_id: id, title: mc.title, price: mc.price, status: 'pending' }])
    if (error) { showNotification('Ошибка при записи'); return }
...
async function saveCourse(e) {
    ...
    const courseData = {
        ...
        students: 0, rating: 0, featured: false
    }
    if (id) await supabaseClient.from('courses').update(courseData).eq('id', parseInt(id))
    else await supabaseClient.from('courses').insert([courseData])
    await loadAllData()
    await renderAllCourses()
    await renderFeaturedCourses()
...
async function saveMasterclass(e) {
    ...
    if (id) await supabaseClient.from('masterclasses').update(mcData).eq('id', parseInt(id))
    else await supabaseClient.from('masterclasses').insert([mcData])
    await loadAllData()
    await renderMasterclasses()
...
async function submitReview() {
    ...
    await supabaseClient.from('reviews').insert([{ user_name: name, rating, text, date: new Date().toISOString().split('T')[0], approved: true }])
    await loadAllData()
    await renderReviews()
    await renderReviewsSummary()
...
function getStars(rating) {
    let stars = ''
    for (let i = 0; i < 5; i++) stars += i < Math.floor(rating) ? '<i class="fas fa-star"></i>' : (i < rating ? '<i class="fas fa-star-half-alt"></i>' : '<i class="far fa-star"></i>')
    return stars
}
...
function continueCourse(courseId) { showNotification('Продолжение курса (в разработке)') }
...
function initFilters() {
    ...
    document.getElementById('searchInput')?.addEventListener('input', () => { currentCoursesPage = 1; renderAllCourses() })
    document.getElementById('priceSelect')?.addEventListener('change', () => { currentCoursesPage = 1; renderAllCourses() })
    document.getElementById('ratingSelectFilter')?.addEventListener('change', () => { currentCoursesPage = 1; renderAllCourses() })
    document.getElementById('sortBySelect')?.addEventListener('change', () => { currentCoursesPage = 1; renderAllCourses() })
    document.getElementById('resetFilters')?.addEventListener('click', () => { document.querySelector('.filter-tab[data-category="all"]').click(); document.getElementById('searchInput').value = ''; document.getElementById('priceSelect').value = 'all'; document.getElementById('ratingSelectFilter').value = '0'; document.getElementById('sortBySelect').value = 'default'; currentCoursesPage = 1; renderAllCourses() })
}

function initRatingSelect() {
    const stars = document.querySelectorAll('#ratingSelect span')
    stars.forEach(star => star.addEventListener('click', function() { const rating = parseInt(this.dataset.rating); document.getElementById('reviewRating').value = rating; stars.forEach((s, i) => i < rating ? s.classList.add('active') : s.classList.remove('active')) }))
}
